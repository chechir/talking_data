...
for fold_id in xrange(1, n_folds + 1):
    ...
    fit = model.fit_generator(
        ...
        nb_epoch=20,
        samples_per_epoch=176 * 381,
        verbose=2,
        validation_data=(Xva.todense(), Yva_dum),
    )

    # evaluate the model
    pred[valid_id, 12:25] = model.predict_generator(
        generator=batch_generatorp(Xva, 400, False), val_samples=Xva.shape[0]
    )
    pred_test[:, 12:25] = pred_test[:, 12:25] + model.predict_generator(
        generator=batch_generatorp(Xtest, 400, False), val_samples=Xtest.shape[0]
    )
...
pred_train_df.to_csv(
    "preds/keras_pred_train_bags5_20160819.csv", index=True, index_label="device_id"
)
pred_test_df.to_csv(
    "preds/keras_pred_test_bags_SVD20_20160819.csv", index=True, index_label="device_id"
)
...
submission = pd.DataFrame(
    pred_test[:, 12:25], index=gatest.index, columns=targetencoder.classes_
)
