...
for j in range(nbags):
    ...
    ## training
    fit = model.fit_generator(
        generator=batch_generator(Xtrain_ne, dummy_y, 381, True),
        nb_epoch=nepoch,
        samples_per_epoch=Xtrain_ne.shape[0],
        verbose=2,
    )
    ## prediction
    pred_test[test_id_ne, 12:25] += model.predict_generator(
...
for j in range(nbags):
    ...
    ## training
    fit = model.fit_generator(
        generator=batch_generator(Xtrain, dummy_y, 381, True),
        nb_epoch=nepoch,
        samples_per_epoch=Xtrain.shape[0],
        verbose=2,
    )
    ## prediction
    pred_test[test_id_we, 12:25] += model.predict_generator(
