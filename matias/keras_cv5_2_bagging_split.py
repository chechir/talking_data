...
folds=pd.read_csv(os.path.join(datadir,"folds_5.csv"), index_col='device_id')
...
print('All features after dimensionality reduction NE: train shape {}, test shape {}'.format(Xtrain_ne.shape, Xtest_ne.shape))
...
nbags=5
nepoch=20
print("Starting training Logistic and Keras. Using {} folds, bagged {} times".format(n_folds, nbags))
...
for fold_id in xrange(1, n_folds + 1):
    ...
    for j in range(nbags):
        ...
        fit = model.fit_generator(generator=batch_generator(Xtr, Ytr_dum, 381, True),
                                  nb_epoch=nepoch,
                                  samples_per_epoch=Xtr.shape[0], verbose=0
                                  )
    ...
    pred[valid_id_we, 12:25] /= nbags

    score_val = log_loss(y[valid_id_we], pred[valid_id_we, 12:25])
    print("Total: Keras-w/e logloss for fold {} is {}".format(fold_id, score_val))

    # evaluate the model across all fold (with and without events)
    Xva, Yva = Xtrain[valid_id, :], y[valid_id]
    score_val = log_loss(Yva, pred[valid_id, 12:25])
...
##Predicting the test set - No Events
#pred_test = np.zeros((gatest.shape[0],nclasses*2))
np.random.seed(seed)
for j in range(nbags):
    model = baseline_model2(Xtrain_ne.shape[1])
    ## training
    fit = model.fit_generator(generator=batch_generator(Xtrain_ne, dummy_y, 381, True),
                              nb_epoch=nepoch,
                              samples_per_epoch=Xtrain_ne.shape[0], verbose=2
                              )
    ## prediction
    pred_test[test_id_ne, 12:25] += model.predict_generator(generator=batch_generatorp(Xtest_ne[test_id_ne,:], 400, False),
                                                        val_samples=Xtest_ne[test_id_ne,:].shape[0])
    print("bagg test no events:", j)

pred_test[test_id_ne, 12:25] /= nbags

#Predicting on the test set - With events- ALL DATA
for j in range(nbags):
    model = baseline_model(Xtrain.shape[1])
    ## training
    fit = model.fit_generator(generator=batch_generator(Xtrain, dummy_y, 381, True),
                              nb_epoch=nepoch,
                              samples_per_epoch=Xtrain.shape[0], verbose=2
                              )
    ## prediction
    pred_test[test_id_we, 12:25] += model.predict_generator(generator=batch_generatorp(Xtest[test_id_we,:], 400, False),
                                                        val_samples=Xtest[test_id_we,:].shape[0])
    print("bagg test with events:", j)

pred_test[test_id_we, 12:25] /= nbags


##Predicting the test set Regression- No Events
clf1 = LogisticRegression(C=0.06, multi_class='multinomial', solver='lbfgs')  # 2.38715733092
clf1.fit(Xtrain_ne, y)
pred_test[test_id_ne, 0:12] = clf1.predict_proba(Xtest_ne[test_id_ne,:])

##Predicting the test set Regression- With Events
clf2 = LogisticRegression(C=0.016, multi_class='multinomial', solver='lbfgs')  # 1.99914889909
clf2.fit(Xtrain, y)
pred_test[test_id_we, 0:12] = clf2.predict_proba(Xtest[test_id_we,:])
...
pred_train_df.to_csv('preds/keras_cv5_2_bagging_split_train.csv', index=True, index_label='device_id')
pred_test_df.to_csv('preds/keras_cv5_2_bagging_split_test.csv', index=True, index_label='device_id')
...
print("Mixed keras-logistic: logloss for {} folds is {}". format(n_folds, score_val))
...
submission.to_csv('preds/keras_cv5_2_bagging_split_mix.csv',index=True)


print(submission.describe())
print(pred_test_df.describe())
