...
nbags = 10
scores = np.zeros((test_sp.shape[0], 12))

for i in range(nbags):
    model = nn_model()
    fit = model.fit_generator(generator = batch_generator(train_sp, ytrain, 400, True),
                              nb_epoch = 18,
...
df.to_csv(PATH + '/preds/keras_3_test.csv', index = True, index_label = 'device_id')
